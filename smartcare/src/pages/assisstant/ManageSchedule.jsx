...
function ManageSchedule() {
  ...
  useEffect(() => {
    ...
    console.log(divRef.current.querySelector("ul").offsetHeight);
      ...
      mealSchedule.map((schedule) => (
        <div key={schedule.id} className="flex-1 py-4">
          ...
          <div
            ...
            style={{ height: "calc(100% - 60px)",background: `${moment().format('DD/MM/YYYY') == moment(schedule.date).format("DD/MM/YYYY") ? "red" : "transparent"}` }}
          >
            <div className="border p-2 h-[33%]">
              {schedule.morning?.split(",")?.map((item, index) => (
            ...
            <div className="border p-2 h-[33%]">
              {schedule.afternoon.split(",")?.map((item, index) => (
                <div key={index} className="ml-4">
                  {index === 0 ? item : <>{item}</>}
                </div>
              ))}
            ...
            <div className="border p-2 h-[33%]">
              {schedule.noon?.split(",")?.map((item, index) => (
                <div key={index} className="ml-4">
                  {index === 0 ? item : <>{item}</>}
                </div>
              ))}
      ...
      <div className="shadow p-[12px] bg-[#ccc]" ref={divRef}>
        ...
        <div className="flex justify-between" ref={tableRef}>
          <div className="p-4">
            ...
            >
              <div className="flex items-center h-[33%] font-semibold">Sáng</div>
              <div className="flex items-center h-[33%] font-semibold">Chiều</div>
